...
// pulling the player package's Svelte/DOMPurify dependency tree into the
// Workers bundle. Keep the at:// resolution logic in sync with that file.
...
async function fetchAtRecord(
  ...
  const cid = timeline.settings?.ogImage?.blobRef?.ref?.$link;
  if (cid && !timeline.settings?.ogImage?.url) {
    timeline.settings = {
      ...timeline.settings,
      ogImage: {
        ...timeline.settings?.ogImage,
...
export async function loadTimeline(source: string, config: LoaderConfig = {}): Promise<LoadResult> {
  ...
  try {
    ...
    return { ok: false, error: 'Expected an at:// URI or https:// URL' };
  } catch (err) {
